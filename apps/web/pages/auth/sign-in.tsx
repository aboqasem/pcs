...
import { usePost, usePrevious, useValidated } from '@/lib/hooks';
import { redirectIf } from '@/lib/services/auth.service';
import { SignInUserDto } from '@myplatform/shared-data-access';
import { GetServerSideProps, InferGetServerSidePropsType } from 'next';
import Head from 'next/head';
import { useRouter } from 'next/router';
import React, { ChangeEventHandler, FormEventHandler, useEffect, useRef } from 'react';
...
export default function SignIn(props: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  const prevPushed = usePrevious(pushed);
  ...
  const {
    ...
    errors,
    setErrors,
  } = useValidated(SignInUserDto, {
    username: '',
    password: '',
    rememberMe: false,
  });
  const { sendRequest, data: userData, error, isPending } = usePost(BffPath.SignIn);

  const onTextFieldChange: ChangeEventHandler<HTMLInputElement> = (e) => {
    setSignInData((curr) => ({ ...curr, [e.target.id]: e.target.value }));
  };

  const onCheckboxChange: ChangeEventHandler<HTMLInputElement> = (e) => {
    setSignInData((curr) => ({ ...curr, [e.target.id]: e.target.checked }));
  };
  ...
  useEffect(() => {
    if (userData) {
      ...
      if (!pushed.current) {
        ...
        console.error('A problem (most probably with authentication cookies) occurred');
      }
    }
  ...
  return (
    <div className="flex flex-col justify-center min-h-screen py-12 bg-gray-50 sm:px-6 lg:px-8">
      <Head>
        <title>Sign in to MyPlatform</title>
      </Head>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="relative px-4 py-8 bg-white shadow sm:rounded-lg sm:px-10">
          ...
          {(isPending || userData) && (
            <Overlay>
              {isPending && <LoadingSpinner className="w-10 h-10 text-blue-700" />}
              {userData &&
                (prevPushed?.current && pushed.current ? (
                  <HiXCircle className="w-10 h-10 text-red-700" />
                ) : (
